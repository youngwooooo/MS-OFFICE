...
<div id="body">
	...
	<div class="row">
			...
			<table class="table">
				...
				<tbody>
					<tr>
						...
						<td>
							<div style="padding: 8px; width: 100%; height: calc(2.25rem + 2px); border-radius: .25rem; border: 1px solid #ced4da;">${comNoticeDetail.sj}</div>
						</td>
					</tr>
					<tr>
						<th class="th"><span>공지 내용</span></th>
						<td>
							<textarea style="padding: 8px; border-radius: .25rem; border: 1px solid #ced4da; resize:none;" class="form-readOnly" rows="10" id="cn" name="cn">${comNoticeDetail.cn}</textarea>
		...
		<div class="divNoticeBtn" id="divFooter2" >
			<c:if test="${comNoticeDetail.wrterEmpno == empVo.empno}">
				<button id="btnNoticeUpdate" type="button" class="btn" onclick="location.href='/com/noticeupdate?cmmntyNo=${comNoticeDetail.cmmntyNo}&bbscttNo=${comNoticeDetail.bbscttNo}'">수정</button>
			</c:if>
			<button id="btnNoticePrev" type="button" class="btn" onclick="location.href='/com/noticeListAdd?cmmntyNo=${comNoticeDetail.cmmntyNo}'">목록</button>
			<c:if test="${comNoticeDetail.wrterEmpno == empVo.empno}">
