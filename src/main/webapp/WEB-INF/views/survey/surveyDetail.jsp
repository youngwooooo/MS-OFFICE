...
<style>
	...
	.rspnsNotOpen {text-align: center; padding: 150px;}
	.rspnsNotOpen i {color: red;}
	...
	.surveyCommentList li {min-height: 100px; margin-bottom: 5px;  word-break:break-all; overflow: hidden;}
...
<script type="text/javascript">
	$(function(){
		...
		$("#btnSurveyCommentAdd").on("click", function(){
			...
			var wrterNm = "${sessionScope.empVo.nm}";
			var ofcpsNm = "${sessionScope.empVo.ofcpsNm}";
				...
				, success : function(result){
					...
					if(result > 0){
						var str = '<li onmouseover="fn_UDshow(this)">'
								...
								+			'</div>'
								+			'<span class="wrter">'+ wrterNm +'&nbsp;'+ ofcpsNm +'</span>'
								+			'<p>'+ answerCn +'</p>'
								+			'<span>'+ writngTm +'</span>'
		...
		// 댓글 쓰고 엔터 시 댓글 등록
		$("input[name=answerCn]").keydown(function (e) {
	        if(e.keyCode == 13){
	        	$("#btnSurveyCommentAdd").click();
	        }
		  ...
		  $("#btnSurveySubmit").on("click", function(){
			  ...
			  $("#surveySubmitFrm input[name='qustnrNo']").val(qustnrNo);
			  $("#surveySubmitFrm input[name='qesitmNoArr']").val(qesitmNoArr);
			  $("#surveySubmitFrm input[name='rspnsExNoArr']").val(rspnsExNoArr);
			  $("#surveySubmitFrm input[name='rspnsCnArr']").val(rspnsCnArr);
			  $("#surveySubmitFrm input[name='rspnsScoreArr']").val(rspnsScoreArr);
			  ...
			  if(rspnsExNoArr.length != qesitmNoObj.length){
 			  ...
 			  $("#surveySubmitFrm").submit();
			  
		  });
		  
		  // 설문 결과 차트 그리기
		  $.ajax({ 
			 url : "/survey/resultChart"
			 , type : "post"
			 , data : {"qustnrNo" : ${detailSurveyList[0].qustnrNo}}
			 , success : function(data){
				 console.log(data);
				 
				 <c:forEach var="survey" items="${detailSurveyQesitm}">
				 <c:if test="${survey.qesitmType == 'QE01'}">
				 var labelsArr = new Array();
				 for(var i=0; i<data.length; i++){
					 if(data[i].qesitmNo == ${survey.qesitmNo}){
						 labelsArr.push(data[i].exCn + "(" + data[i].rspnsCount + "명" + ")" );	 
					 }
				 }
				 ...
				 for(var i=0; i<data.length; i++){
					 if(data[i].qesitmNo == ${survey.qesitmNo}){
						 dataArr.push(data[i].rspnsCount);
					 }
				 }
				 console.log(dataArr);
				 ...
				 for(var i=0; i<labelsArr.length; i++){
					 backgroundColorArr.push("#" + Math.floor(Math.random() * 16777215).toString(16));
				 }
				 console.log(backgroundColorArr);
				 
				// 설문 결과 차트 만들기
				var ctx = document.getElementById("myChart${survey.qesitmNo}");
...
<div id="body">
	...
	<div class="lefr-menu-list-content">
		...
		<div class="surveyDetailContent">
			<div class="surveyInfo">
				...
				<div class="divSurveyDesc">
					...
					<c:if test="${extsn == 'jpg' || extsn == 'png' || extsn == 'jpeg' || extsn == 'jfif' || extsn == 'bmp'}">
						<div class="col-sm-3">
							<img src="/reload2?fileNo=${surveyFileImg.fileNo}&fileOrdr=${surveyFileImg.fileOrdr}" alt="${surveyFileImg.fileOrginlNm}">
						</div>
					</c:if>
					</c:forEach>
					</div>
			...
			<c:forEach var="detail" items="${detailSurveyTrgter}">
				<c:if test="${detail.trgterEmpno == sessionScope.empVo.empno}">
					<c:set var="wrterEqulTegter" value="${detail.trgterEmpno}" />
				</c:if>
			</c:forEach>
			
			<!-- 설문 보기를 보여줌(설문 대상자가 응답하지 않았고 설문 대상자가 존재할 때) -->
			<c:if test="${getRspnsAt.rspnsAt != 'Y'&& wrterEqulTegter != null}">
			<!-- 설문  시작 -->
			<div class="divAddQuestionResultList">
				...
				<!-- 작성자라도 대상자에 포함되지 않았다면 설문 제출 불가 -->
				<c:if test="${detailSurveyList[0].qustnrStatus == 'QU01' && wrterEqulTegter != null}">				
				<div class="btnSurveySubmit">
					<button type="button" class="btn" id="btnSurveySubmit">설문 제출</button>
				</div>
				</c:if>
			...
			<!-- 응답 제출을 위한 form -->
			<form id="surveySubmitFrm" action="/survey/surveySubmit" method="post">
			...
			<!-- 설문 결과를 보여줌(설문에 대한 응답을 했을 때, 설문 결과 공개 여부가 공개일 때, 설문 대상자가 아니고 설문 작성자가 로그인한 사원의 사번과 같을 때) -->
			<c:if test="${getRspnsAt.rspnsAt == 'Y' && detailSurveyList[0].othbcAt == 'Y' or wrterEqulTegter == null && detailSurveyList[0].wrterEmpno == sessionScope.empVo.empno}">
			
			<div class="surveyResultList">
				<!-- 설문 결과(점수형) 시작 -->
				<c:forEach var="list" items="${detailSurveyQesitm}">
				<c:if test="${list.qesitmType == 'QE02'}">
				<div class="surveyPointTypeResult">
					<p>${list.qesitmNo}.&nbsp;<span>${list.cn}</span></p>
					<div class="surveyResult">
						<span>전체 참여자<strong>${detailSurveyTrgter.size()}</strong></span>
						<span>참여자<strong>${rspnsEmpnoList.size()}</strong></span>
						<c:set var="resultPercent" value="${rspnsEmpnoList.size() / detailSurveyTrgter.size()}" />
						<span>참여율<strong class="resultPercent"><fmt:formatNumber value="${resultPercent}" type="percent" /></strong></span>
						<c:forEach var="sum" items="${getSumScore}">
						<c:if test="${sum.qesitmNo == list.qesitmNo}">
						<c:set var="avgScore" value="${sum.sumScore / rspnsEmpnoList.size()}" />
						<span>평균점수<strong class="resultPoint"><fmt:formatNumber value="${avgScore}" type="number" pattern="0.0" /></strong></span>
						</c:if>
						</c:forEach>
					</div>
				</div>
				...
				<!-- 설문 결과(선택형) 시작 -->
				<c:if test="${list.qesitmType == 'QE01'}">
				<div class="surveySelectTypeResult">
					...
					<div class="resulrChart">
						<canvas id="myChart${list.qesitmNo}" width="380" height="300"></canvas>
					</div>
				</div>
				</c:if>
				</c:forEach>
				...
				<!-- 응답 수정 버튼 활성화 여부(설문이 참여 후 수정 여부를 허용, 설문이 진행중일 때, 설문 대상자에 해당될 때) -->
				<c:if test="${detailSurveyList[0].afterUpdtAt == 'Y' && detailSurveyList[0].qustnrStatus == 'QU01' && wrterEqulTegter != null}">
				<div class="btnSurveySubmit">
					<button type="button" class="btn" id="btnSurveyResultEdit" onclick="javascript:location.href='/survey/detailEdit/${detailSurveyList[0].qustnrNo}'">응답 수정</button>
				</div>
				</c:if>
			</div>
			...
			<c:if test="${getRspnsAt.rspnsAt == 'Y' && detailSurveyList[0].othbcAt == 'N'}">
				<div class="rspnsNotOpen"><h3><i class="fas fa-exclamation-circle mr-3"></i>결과를 공개하지 않는 설문입니다.</h3></div>
			</c:if>
...
<%-- 							<img src="${url}${fileCours}/${answer.fileNm}"> --%>
