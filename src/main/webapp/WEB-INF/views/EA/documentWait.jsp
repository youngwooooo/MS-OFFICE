...
<script>
	function fn_move(word){
		location.href = "/EA/documentWait?drftProgrsNm="+word+"&currentPage=1&selSearch=${param.selSearch}&keyword=${param.keyword}"
	}
</script>
<div id="body">
						...
						<tbody>
							...
							<c:forEach var="drft" items="${list}">
								<tr>
									...
									<td class="date">
										<span class="txt">${drft.completeTm}</span>
									</td>
									<td class="division">
									...
									<td class="subject">
										<a href="/EA/documentWait/detail?drftNo=${drft.drftNo}" style="color:black;">
					...
					<div class="tool_bar tool_absolute">
						<div class="dataTables_paginate paging_full_numbers">
							<c:if test="${param.currentPage  > 5}">
								<a id="first_page" tabindex="0" href="/EA/documentWait?drftProgrsNm=${param.drftProgrsNm}&currentPage=1&selSearch=${param.selSearch}&keyword=${param.keyword}"
								...
								<a id="prev_page" tabindex="0" href="/EA/documentWait?drftProgrsNm=${param.drftProgrsNm}&currentPage=${paging.startPage - 5}&selSearch=${param.selSearch}&keyword=${param.keyword}"
							...
							<span id="pages"> 
								<c:forEach var="pageNo" begin="${paging.startPage }" end="${paging.endPage }" varStatus="stat">
							        <a class="paginate_button <c:if test="${pageNo == param.currentPage}">paginate_active</c:if>" href="/EA/documentWait?drftProgrsNm=${param.drftProgrsNm}&currentPage=${pageNo}&selSearch=${param.selSearch}&keyword=${param.keyword}" data-dt-idx="${pageNo }" tabindex="0">${pageNo }</a>
					           	</c:forEach>
							</span> 
							<c:if test="${param.currentPage  <= paging.totalPages - paging.totalPages%5}">
								<a id="next_page" tabindex="0" href="/EA/documentWait?drftProgrsNm=${param.drftProgrsNm}&currentPage=${paging.startPage + 5}&selSearch=${param.selSearch}&keyword=${param.keyword}"
									class="next paginate_button "
									title="다음">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-right" viewBox="0 0 16 16">
									  <path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/>
									</svg>	
								</a> <a id="last_page" tabindex="0" href="/EA/documentWait?drftProgrsNm=${param.drftProgrsNm}&currentPage=${paging.totalPages}&selSearch=${param.selSearch}&keyword=${param.keyword}"
...
function fn_search() { 
	if (window.event.keyCode == 13) {
		...
		location.href = "/EA/documentWait?currentPage=1&selSearch=" + searchType + "&keyword=" + keyword;
	}
}	
</script>